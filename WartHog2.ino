...
uint32_t rpm[2] = {0, 0}; //tracking motor speed
int32_t TBH[2] = {0, 0};
int32_t TBHError[2];
float KI = 0.1;
int32_t TBHOutput[2];
int32_t TBHErrorPrior[2] = {1, 1};
bool TBHReset = true;
...
void setup1(){
  esc[0] = new BidirDShotX1(escPin1, 600);
  esc[1] = new BidirDShotX1(escPin2, 600);
...
void loop() {
 ...
 switchPosPrev = switchPos;
  ...
  if (!BUTTONHIGH && escThrottle[0] == 0 && pushState == IDLE) {                   // If encoder button is pressed, ground signal sent, also ensure motor and solenoid are at rest.
  ...
  if(dartQueue > 0){  //darts in queue, move to main firing actions
    ...
    power1 = motorspeedSetting;
    power2 = motorspeedSetting;
    currentThrottle = power1 * 8; //for braking calc change
    if(motorStabilized){
...
void loop1() { //motor core, should be core1 in main code
  ...
  for(int i = 0; i < 2; i++){
    esc[i]->getTelemetryErpm(&rpm[i]);
    rpm[i] /= MOTOR_POLES / 2; // eRPM = RPM * poles/2
  }

  if(power1 > idleSetting){        //spinup control, compares %, EX: 50% > 20%
    //escThrottle = power1 * 20;    // Converting from 1-100% to 0 - 2000 range.
    //RPM packet was captured already
    idleCap = 0;
    for(int i = 0; i < 2; i++){
      if(rpm[i] < (expectedRpm * 0.75)){
        escThrottle[i] = 1000;
      }
      else{
        escThrottle[i] = (power1 * 8); //change
      }

      if(escThrottle[i] > 1501){
        escThrottle[i] = 1500;
      }
      if(escThrottle[i] < 0){
        escThrottle[i] = 0;
  ...
  for(int i = 0; i < 2; i++){
    if(escThrottle[i] >= 1 && idleCap == 0){ //start braking
      if(escThrottle[i] <= (idleSetting * 8) + (2 * round((currentThrottle - idleSetting * 8) / (desiredBrakeTime[brakeSetting] /7.2)))){ //calc to smooth out lower end stutter CHANGE

        escThrottle[i] = (idleSetting * 8); //CHANGE
        idleCap = 1;
      }
      delayMicroseconds(7000); //decrement every 7ms move to outside for loop
      brakeIncrement = round((currentThrottle - idleSetting * 8) / (desiredBrakeTime[brakeSetting] / 7.2)); //linear match for brake amount to speed to keep in time CHANGE
      ...
      escThrottle[i] -= brakeIncrement;
      
    }
    // SAFETY CHECK.
    if(escThrottle[i] > 2000 || escThrottle[i] <= 0){ //>2000 is when the escThrottle overflows to its max value because of 0-1 on the int, < 0 is for the motorStabilized flag
      //motorStabilized = false; //should be stopped so motors arent stabilized
      escThrottle[i] = 0; //make sure to drop escThrottle
      //rpm = 0; //reset rpm as it doesn't automatically
    }
  }  
  }
  for(int i = 0; i < 2; i++){
    esc[i]->sendThrottle(escThrottle[i]); //main thing that turns on motor
  }
  ...
  if(rpm[0] >= (expectedRpm - 400) || rpm[1] >= (expectedRpm - 400)){
