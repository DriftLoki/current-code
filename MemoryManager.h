...
#include <EEPROM.h>       // Works with Pi pico via arduino-pico core, must call EEPROM.begin() and commit(). 
...
int brakeSetting;             // Brake amount, used as # with values 0 through 3, the higher the number the faster the motors will break.
int hangtimeSetting;             // Time before motor revs down after trigger release, in ms.
int idleSetting;             // Tournament mode On or Off
...
int* modifierArray[7] = { &dpsSetting, &motorspeedSetting, &brakeSetting, &hangtimeSetting, &idleSetting, &modeSetting, &burstSetting };  // Array of pointers to modify values in the settings menu.
...
void loadvalues(uint8_t loadPosition) {   
  ...
  switch(loadPosition) {
    ...
    brakeSetting = EEPROM.read(brake_for);
    hangtimeSetting = EEPROM.read(hang_for) * 100;  
    idleSetting = EEPROM.read(idle_for); 
    ...
    brakeSetting = EEPROM.read(brake_mid);
    hangtimeSetting = EEPROM.read(hang_mid) * 100; 
    idleSetting = EEPROM.read(idle_mid); 
    ...
    brakeSetting = EEPROM.read(brake_rear);
    hangtimeSetting = EEPROM.read(hang_rear) * 100; 
    idleSetting = EEPROM.read(idle_rear); 
...
void savevalues(int savePosition) { 
  ...
  switch(savePosition) {                               // Passed position determines which switch position to update to, EEPROM.update causes erratic behaviour as entire flash sector needs to be reset.
    ...
    EEPROM.write(brake_for, brakeSetting); 
    EEPROM.write(hang_for, (hangtimeSetting / 100));
    EEPROM.write(idle_for, idleSetting); 
    ...
    EEPROM.write(brake_mid, brakeSetting); 
    EEPROM.write(hang_mid, (hangtimeSetting / 100));
    EEPROM.write(idle_mid, idleSetting); 
    ...
    EEPROM.write(brake_rear, brakeSetting); 
    EEPROM.write(hang_rear, (hangtimeSetting / 100));
    EEPROM.write(idle_rear, idleSetting); 
